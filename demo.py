...
from src.models.image_w_attn import ImageWithAttentionModel
    ...
    def load_model(self, weights_path: Optional[str] = None,
        ...
        default_configs = {
            'hybrid': {
                ...
                'backbone': 'resnet50', 'numerical_features_dim': 5,
                'fusion_method': 'multihead_cross_attention',
                'dropout_rate': 0.1, 'use_mask_attention': True
            },
            'image_only': {
                'spatial_dims': 3, 'in_channels': 1, 'num_classes': 2,
                'backbone': 'resnet50', 'dropout_rate': 0.1, 'pretrained': False
            },
            'image_w_attn': {
                'spatial_dims': 3, 'in_channels': 1, 'num_classes': 2,
                'backbone': 'resnet50', 'dropout_rate': 0.1, 'use_mask_attention': True
            }
        }
        ...
        if self.model_type == 'hybrid':
            ...
            self.model = ImageWithAttentionModel(**config)
        else:
    ...
    def run_inference(self) -> Dict:
        ...
        with torch.no_grad():
            for batch in tqdm(self.data_loader, desc="Inference"):
                ...
                predictions = (probabilities[:, 1] >= self.classification_threshold).long()
...
def main():
    ...
    parser.add_argument('--classification_threshold', type=float,
                       default=DEFAULT_CLASSIFICATION_THRESHOLD,
                       help='Threshold for positive rapid-decline predictions')
